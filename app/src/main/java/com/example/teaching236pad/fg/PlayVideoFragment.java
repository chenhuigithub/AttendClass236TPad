...
import com.example.teaching236pad.adapter.VideoCatalogAdapter;
import com.example.teaching236pad.application.CustomApplication;
...
import com.example.teaching236pad.util.DensityUtil;
import com.example.teaching236pad.util.FormatUtils;
...
//        myApplication = (CustomApplication)( getActivity().getApplication());
    ...
    private void changeVedioSize() {
        ...
        if (getActivity().getRequestedOrientation() == ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) { // 切换至横屏
            ...
            if (vedioHeight >= vedioWidth) {
                autoheight = CustomApplication.Screen_Width;
                autoWidth = (int) (autoheight * scale);
            } else {
                // autoWidth = myApplication.Screen_Height;
                // autoheight = (int) (autoWidth / scale);
                autoWidth = CustomApplication.Screen_Height;
                autoheight = CustomApplication.Screen_Width;
            }
        } else { // 切换至竖屏
            if (vedioHeight >= vedioWidth) {
                autoheight = DensityUtil.dip2px(context, 240);
                autoWidth = (int) (autoheight * scale);
            } else {
                // autoWidth = myApplication.Screen_Width;
                // autoheight = (int) (autoWidth / scale);
                autoWidth = CustomApplication.Screen_Width;
                autoheight = DensityUtil.dip2px(context, 240);
            }
        }
    ...
    private void initListener() {
        ...
        lstvVideo.setOnItemClickListener(new OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> parent, View v, int pos,
                                    long id) {
                VideoAudio info = infoList.get(pos);
    ...
    private void loadVideo(String movieUri, String playType) {
        if (ValidateFormatUtils.isEmpty(movieUri)) {
            Toast.makeText(context, "播放的网址不存在", Toast.LENGTH_SHORT)
                    .show();
            return;
        }
        ...
        try {
            ...
            player.setDataSource(context, Uri.parse(movieUri)); // 加载视频
